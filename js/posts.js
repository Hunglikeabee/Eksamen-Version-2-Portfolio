    ...
    function makePostPage(countPages, lengthPages) {
      ...
      for (let i = countPages; i < lengthPages; i++) {
        ...
        posts.innerHTML += `<a href="post.html?id=${resultPosts[i].id}">
                                    ...
                                    <div class="posts-image" style="background-image: url(${resultPosts[i]._embedded["wp:featuredmedia"][0].source_url})"></div>
    ...
    function selectSort() {
      if (sortBy.value == "difficulty") {
        function fixOrder(a, b) {
          if (
            a._embedded["wp:term"][1][0].name <
            b._embedded["wp:term"][1][0].name
          ) {
            return -1;
          }
          if (
      ...
      if (sortBy.value == "time") {
        function fixOrder(a, b) {
          if (
            a._embedded["wp:term"][1][1].name <
            b._embedded["wp:term"][1][1].name
          ) {
            return -1;
          }
          if (
