    ...
    for (let i = 0; i < resultPosts.length; i++) {
      if (resultPosts[i].id == id) {
        ...
        const postContainer = document.querySelector(
          `.postid-${resultPosts[i].id}`
        );

        for (let j = 0; j < resultMedia.length; j++) {
          if (resultMedia[j].post === resultPosts[i].id) {
            postContainer.innerHTML += `<div class="image-caption"><img alt="${resultMedia[j].alt_text}" class="id${resultMedia[j].post} modal" src="${resultMedia[j].source_url}">
...
async function sendTheForm(post, name, text) {
  const commentsAPI = `https://hunglikeabee.com.one/project-exam-1-hunglikeabee.com/wp-json/wp/v2/comments`;
